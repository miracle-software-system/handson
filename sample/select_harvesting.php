...
<head>
	...
	function getState(val){
		...
		//alert( block_name);
		$.post(
				"select_palm_harvest.php",
				{palmno: palmno,seed_val: seed_val,block_name: block_name},
				function(response){
						$('#bagg_pollination_harvest').html(response);
				}
			);
...
<div>
			...
			<select id="Palm_No_harvest-list" onChange="getState(this.value)" style="background: #2196F3;color: #fff;padding: 10px;width: 120px;height: 40px;border: none;font-size: 15px;">
				...
				$sql = "select distinct(palmno),pollenationdt from femalebagging where sgid = '".$_REQUEST["seed_val"]."' && blkid = '".$_REQUEST["block_name"]."' && dtofharvest IS NULL";
				...
				<!--
					...
					<option><?php echo $rs["pollenationdt"]; ?></option>
					<option><?php echo $date_interval;?></option>
				-->
					<option id="va_h" value="<?php echo $rs["palmno"]; ?>"><?php echo $rs["palmno"];?> </option>
				<?php
				}
				}
				}
...
<div id="bagg_pollination_harvest" style="display: inline-block;float: left;"></div>
